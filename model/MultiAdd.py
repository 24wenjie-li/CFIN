...
class PatchEmbed(nn.Module):
    def __init__(self, img_size, patch_size=4, in_chans=32, embed_dim=32, norm_layer=None):
        super().__init__()
        ...
        if norm_layer is not None:  ## norm_layer=None
...
class Conv2d_CG(nn.Conv2d):
    def __init__(self, in_channels=64, out_channels=64, kernel_size=1, padding=0, stride=1, dilation=1, groups=1,
        ...
        #Parameter(torch.zeros(out_channels, in_channels, kernel_size, kernel_size),
        #                             requires_grad=True).cuda()
        self.bias_conv = Parameter(torch.Tensor(out_channels))#Parameter(torch.zeros(out_channels), requires_grad=True)
...
class SCConv(nn.Module):
    def __init__(self, n_feats, wn=lambda x: torch.nn.utils.weight_norm(x)):
        ...
        pooling_r = 2
        med_feats = n_feats // 1
        #self.k = nn.Conv2d(n_feats, n_feats, kernel_size=1, stride=2, padding=0, groups=1)
                                    
        self.k1 = nn.Sequential(nn.ConvTranspose2d(n_feats, n_feats*4//3, kernel_size=pooling_r, stride=pooling_r, padding=0, groups=1, bias=True),
                                nn.LeakyReLU(0.05),
...
class ConvBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.rb_search1(x)
        x = self.squeeze_block(self.trans_block(self.expand_block(x))) + x
        
        x = self.rb_search1(x)
        x = self.squeeze_block(self.trans_block1(self.expand_block(x))) + x
        
        x = self.rb_search2(x)
        x = self.squeeze_block1(self.trans_block2(self.expand_block1(x))) + x
        
        x = self.rb_search2(x)
        x = self.squeeze_block1(self.trans_block3(self.expand_block1(x))) + x
        
        x = self.rb_search3(x)
        x = self.squeeze_block2(self.trans_block4(self.expand_block2(x))) + x
        
        x = self.rb_search3(x)
        x = self.squeeze_block2(self.trans_block5(self.expand_block2(x))) + x
        
        x = self.rb_search4(x)
        x = self.squeeze_block3(self.trans_block6(self.expand_block3(x))) + x
        
        x = self.rb_search4(x)
        x = self.squeeze_block3(self.trans_block7(self.expand_block3(x))) + x
...
class ConvTransBlock(nn.Module):
    ...
    def __init__(self, inplanes, outplanes, res_conv, stride, dw_stride, embed_dim, num_heads, mlp_ratio,
        ...
        #self.fusion_block = ConvBlock(inplanes=inplanes, outplanes=outplanes, res_conv=res_conv, stride=1, groups=groups)
...
class MODEL(nn.Module):
    def __init__(self, norm_layer=nn.LayerNorm, patch_size=1, window_size=8, num_heads=8, mlp_ratio=1.,
