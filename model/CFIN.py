...
class IGP(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.fc1 = CGM(in_features, hidden_features, kernel_size=1, padding=0)
        self.act = act_layer()
...
class CFGT(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        kernel_size2 = 3
        padding2 = 1
        self.attn = CGA(dim, num_heads, kernel_size1, padding1)
        self.attn1 = CGA(dim, num_heads, kernel_size2, padding2)

        self.norm2 = LayerNorm(dim)
        self.norm3 = LayerNorm(dim)
        mlp_hidden_dim = int(dim*1)
        self.mlp = IGP(in_features=dim, hidden_features=mlp_hidden_dim)
...
class CTGroup(nn.Module):
    def __init__(self, inplanes, outplanes, stride=1, res_conv=False, act_layer=nn.ReLU, groups=1, norm_layer=nn.BatchNorm2d, drop_block=None, drop_path=None):
        ...
        self.rb_search1 = CIAM(med_planes)
        self.rb_search2 = CIAM(med_planes)
        self.rb_search3 = CIAM(med_planes)
        self.rb_search4 = CIAM(med_planes)
...
class ConvTransBlock(nn.Module):
    ...
    def __init__(self, inplanes, outplanes, res_conv, stride, dw_stride, embed_dim, num_heads, mlp_ratio,
        ...
        expansion = 1
        self.cnn_block = CTGroup(inplanes=inplanes, outplanes=outplanes, res_conv=res_conv, stride=1, groups=groups)
